...
#:             : Installs Oracle's JDK8 in an Atos NOCUY Centos installation,
#:             : from payload's zip compressed tarball.
...
_JDKPYLDVRS=1.8.0_XXX                             ## Payload's jdk version
...
setJdkPyldVers() { #@ DESCRIPTION: Sets global _JDKPYLDVRS with payload's JDK8
    ...
    # Local variables ("local" must be function's first statement)
    local vers r                 ## Jdk version and STDIN lines counter
    r=0                          ## Initialize readed lines counter
...
uninstallJdk8() { #@ DESCRIPTION: Removes Oracle JDK8 installation directory
    ...
    while IFS= read -r _RPLY; do           ## Read whole current line
        [ X"$_RPLY" = Xprintf ] ||         ## Reset curor's pos. and println
...
EOF
    ...
    if [ -e "$_SHPRFLENME" ]; then             ## Chek if it still exists
        errmsg "Failed to remove $_SHPRFLENME" ## If so, something whent wrong
        noerrs=false
    fi
...
installJdk8() { #@ DESCRIPTION: Installs Oracle's JdK8 from payload
                #@ USAGE: installJdk8
    # Decompress & untar all files to inistallation path in coprocess, and print
    ...
    while IFS= read -r _RPLY; do ## Red whole current line
        ...
        esac                     ## Printf resets cursos position and println
...
$({ tail -n +$_PYLDLINENM "$0" | 
  ...
  printf "STATUS: %d" $?)
EOF
    ## Coprocess starts reading $0 from payload first line onwards, sending out-
...
installShProfile() { #@ DESCRIPTION: Installs shell profile w/environment provi-
                     #@              ded by new JDK installation. Tipically this
                     #@              it will set your JAVA_HOME.
                     #@ USAGE: installShProfile
    local out             ## Write command output container
    # Tru tp wrote tp $_SHPRFLENME shell profile saving attempts output
...
Status >1 is used for syntas or input/output errors.
...
main() {
        ...
        case $opt in                   ## Check option case
            [Uu])                      ## Uninstall mode selected?
                if $uinm; then         ## If already selected finish w/errors
                    ...
                    return 2           ## (Allow uninstal option only once)
                fi
                uinm=true ;;           ## Else set uninstal mode flag to true
    ...
    shift $(($OPTIND - 1))        ## Shift already parced arguments (opts)
    ...
    if $uinm; then # Uninstall mode --------------------------------------------
        ...
        # Check installation dir. existence before doint anything
        if [ ! -d "$_JDKPRNTDIR/$_JDKPYLDVRS" ]; then
            ...
            then
                ...
                stdmsg "Cancelled"               ## If "No", do not unistall
            ...
            if ynPrmpt "Remove alternatives for common programs"; then
                ...
                stdmsg "Cancelled"               ## Alternatives rmo cancelled
            fi  
        fi      
        ...
        if [ -e "$_SHPRFLENME" ]; then
            ...
            if ! ynPrmpt "Would you like to re-install it"; then
                ...
                if ! installShProfile; then
                    noinsterrs=false
                    stdmsg "Re-install Failed."
                else
                    stdmsg "Re-install Done."
                fi
            fi
        elif ! installShProfile; then
            noinsterrs=false
            stdmsg "Failed."
        fi
        stdmsg "Done."
        ...
        if ynPrmpt "Install alternatives for common programs"; then
            stdmsg "Installing alternatives.."
            sleep 1
            for alt in jar java javac javaws keytool; do
                stdmsg "Alternative for program $alt"
                sleep 0.5
